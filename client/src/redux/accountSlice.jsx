...
export const toggleAutoJob = createAsyncThunk(
  'account/toggleAutoJob',
  async (_, { getState, rejectWithValue }) => {
    try {
      const { user: { token } } = getState();
      if (!token) {
      ...
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.put(`${API_URL}/toggle-auto-job`, {}, config);
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data?.message || error.message);
    }
...
const accountSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(toggleAutoJob.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(toggleAutoJob.fulfilled, (state, action) => {
        state.loading = false;
        state.autoJobEnabled = action.payload.autoJobEnabled;
      })
      .addCase(toggleAutoJob.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      });
  },
