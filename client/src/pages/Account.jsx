...
const Account = () => {
  ...
  const handlePartyInputChange = (selectedOption) => {
    setFormData({ ...formData, partyname: selectedOption ? selectedOption.value : '', transactionType: 'credit' });
    setCurrentPage(1);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        ...
        await dispatch(fetchAccounts()).unwrap();
        if (formData.toParty) {
          setFormData((prev) => ({ ...prev, partyname: formData.toParty, toParty: '', transactionType: 'credit' }));
        } else {
          setFormData({ partyname: formData.partyname, amount: '', transactionType: 'credit', remark: '', date: formData.date, toParty: '' });
        }
      }
