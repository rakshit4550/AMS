...
//   const balValue = balance > 0 ? `-${Math.abs(balance)}` : Math.abs(balance).toString(); // Convert to string
...
//   doc.setFontSize(12);
//   doc.text(`₹ ${balValue} ${balSign}`, boxX + 5, y + 15); // Already a string
//   y += 25;
...
//     const curBalValue = currentBalance > 0 ? `-${Math.abs(currentBalance)}` : Math.abs(currentBalance).toString(); // Convert to string
...
//       doc.text((acc.debit || 0).toString(), tableX + 42, y + 6); // Convert to string
//     }
//     if (acc.credit > 0) {
//       doc.setTextColor(0, 128, 0);
//       doc.text((acc.credit || 0).toString(), tableX + 82, y + 6); // Convert to string
//     }
//     doc.setTextColor(0, 0, 0);
//     doc.text(`${curBalValue} ${curBalSign}`, tableX + 122, y + 6); // Already a string
        ...
        {selectedParty && selectedPartyAccounts.length > 0 ? (
          <div className="mb-8 overflow-x-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl sm:text-2xl font-semibold text-gray-800">{selectedPartyName}</h3>
              <div className={`text-lg font-semibold ${balanceColor}`}>
                Closing Balance: Rs. {balValue} {balSign}
              </div>
            </div>
            <div className="shadow-lg rounded-lg overflow-hidden">
              <table className="w-full border-collapse bg-white">
